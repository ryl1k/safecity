...
create or replace function segment_rating(
  ...
  select case
    ...
    -- Smoothness excellent/good redeems rough surfaces (e.g. smooth cobblestone at Ринок)
    -- Must come before the surface-based none checks.
    when p_smoothness = any (array['excellent','good']) then
        case
    ...
    -- Bad smoothness overrides good surface
    when p_smoothness = any (array['bad','very_bad','horrible','very_horrible','impassable']) then 'none'
    -- Surface-based none (only reached when smoothness is null or intermediate)
    ...
    -- Good surface, smoothness null or not explicitly bad
    when p_surface = any (array['asphalt','concrete','paving_stones','concrete:plates','paved','wood','metal']) then
        case
          when p_width   is not null and p_width < 1.5    then 'partial'
          when p_incline is not null and abs(p_incline) >= 6 then 'partial'
          when p_lit            = false                   then 'partial'
...
try {
  ...
  console.log('segment_rating() updated: good/excellent smoothness now overrides cobblestone → none');
} catch (e) {
