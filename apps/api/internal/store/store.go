...
insert into problems (title, description, category, severity, point_id, geom, photos, created_by)
values (
  $1,
  $2,
  $3::point_category,
...
returning id::text, title, description, category::text, severity, status::text,
          ...
          ST_Y(geom::geometry), ST_X(geom::geometry), photos`
...
func (s *Store) CreateProblem(ctx context.Context, userID string, in NewProblem) (Problem, error) {
	...
	err := s.db.WithUser(ctx, userID, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx, insertProblemSQL,
			...
			&p.Status, &p.Confirmations, &p.PointID, &p.CreatedAt, &p.Lat, &p.Lng, &p.Photos,
		)
	})
	if err != nil {
		return Problem{}, err
