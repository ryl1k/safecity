...
export default function PointDetailPage({ params }: { params: { id: string } }) {
  ...
  // Applicable features for the active profile, critical first (the "why").
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      <main style={{ maxWidth: 760, margin: '0 auto', padding: '1.4em 1.25em 4em' }}>
        ...
        {status === 'notfound' && (
          <p style={{ marginTop: '1.5em', color: 'var(--sc-muted)' }}>Місце не знайдено.</p>
        )}
        ...
        {status === 'ready' && point && (
          <>
            ...
            {/* Per-profile ratings — honest, never one averaged score. */}
            ...
            {/* Why — critical features pass/fail for the active profile */}
            <section style={card}>
              <h2 style={cardTitle}>Чому така оцінка · {profileLabel[primary]}</h2>
              {features.length === 0 ? (
                <p style={{ color: 'var(--sc-muted)', fontSize: '0.9em', margin: 0 }}>Немає критеріїв для цієї категорії.</p>
              ) : (
                features.map((f, i) => (
                  <ChecklistRow
                    key={f.key}
