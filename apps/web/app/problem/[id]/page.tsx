...
export default function ProblemPage({ params }: { params: { id: string } }) {
  ...
  async function confirm() {
    ...
    if (!uid) return;
    await supabase.from('problem_confirmations').insert({ problem_id: params.id, user_id: uid });
    setConfirms((c) => c + 1);
    setConfirmed(true);
  }
  ...
  async function sign() {
    ...
    const uid = await requireUser();
    if (!uid) return;
    await supabase.from('petition_signatures').insert({ petition_id: petition.id, user_id: uid });
  }

  return (
    <div style={{ minHeight: '100vh' }}>
      <AppHeader active="civic" />
      <main style={{ maxWidth: 700, margin: '0 auto', padding: '1.4em 1.25em 4em' }}>
            ...
            {petition && (
              <section style={{ marginTop: '1.6em', background: 'var(--sc-surface)', border: 'var(--sc-bw) solid var(--sc-primary)', borderRadius: '1em', padding: '1.3em' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85em', fontWeight: 700, marginBottom: '0.35em' }}>
                  <span>{petition.internalSignatures} підписів</span>
                ...
                <div role="progressbar" aria-valuenow={petition.internalSignatures} aria-valuemin={0} aria-valuemax={PETITION_GOAL} style={{ height: '0.7em', borderRadius: '1em', background: 'var(--sc-surface-2)', overflow: 'hidden' }}>
                  <div style={{ width: `${Math.min(100, (petition.internalSignatures / PETITION_GOAL) * 100)}%`, height: '100%', background: 'var(--sc-primary)' }} />
                </div>

                <div style={{ display: 'flex', gap: '0.7em', marginTop: '1em', flexWrap: 'wrap' }}>
                  <Button onClick={sign}>Підписати у застосунку</Button>
