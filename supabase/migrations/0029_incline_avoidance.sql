-- Add optional max_incline parameter to route_accessible_v.
-- When supplied, segments where abs(incline_percent) > max_incline receive a
-- 10× cost multiplier so Dijkstra steers around steep sections. Segments with
-- NULL incline_percent are not penalised (unknown slope = passable). The
...
BEGIN
  ...
  -- ── Cost expression and none-filter by mode ──────────────────────────────
  IF mode = 'flat' THEN
  ...
  -- ── Incline penalty (skipped when max_incline IS NULL) ───────────────────
...
GRANT EXECUTE ON FUNCTION public.route_accessible_v(
  double precision, double precision, double precision, double precision, text, float8
