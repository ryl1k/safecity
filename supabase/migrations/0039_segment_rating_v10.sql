-- segment_rating v10
-- Changes vs v9:
--   1. step_free=false no longer hard-blocks when has_ramp=true (ramp compensates for steps)
--   2. has_ramp counts as a partial signal
--   3. surface_type alone (without smoothness) now yields partial for passable surfaces
--      (bad surfaces already fall to none via the terrain check above)
create or replace function segment_rating(
  ...
  p_has_ramp      boolean default null
) returns text language sql immutable as $$
  select case
    -- Hard barriers → none (ramp overrides step_free=false; nothing overrides curb_cuts=false).
    when p_step_free     = false and (p_has_ramp is null or p_has_ramp = false) then 'none'
    ...
    when p_smoothness    is not null
      ...
      or p_has_ramp      is not null
      or p_surface       is not null
    then 'partial'
