...
// route_accessible_v — parameterised by mode:
//   strict   : filter out 'none' segments entirely (current behaviour)
//   moderate : include 'none' at 5× the cost of a 'full' segment
//   flat     : every segment costs just its physical length (ignore rating)
const sql = `
CREATE OR REPLACE FUNCTION public.route_accessible_v(
  ...
  mode      text DEFAULT 'moderate'   -- 'strict' | 'moderate' | 'flat'
...
console.log('✓ route_accessible_v() created');

// Smoke test all 3 modes
for (const mode of ['strict', 'moderate', 'flat']) {
  const t = Date.now();
  const r = await client.query(`SELECT route_accessible_v(24.029, 49.841, 24.035, 49.845, $1) AS r`, [mode]);
  const ms = Date.now() - t;
  const res = r.rows[0]?.r;
  if (res?.error) {
    console.log(`  [${mode}] error: ${res.error}`);
  } else {
    const s = res?.rating_summary ?? {};
    console.log(`  [${mode}] ${Math.round(res?.distance_m)}m  full=${Math.round(s.full_m)}m partial=${Math.round(s.partial_m)}m none=${Math.round(s.none_m)}m  (${ms}ms)`);
  }
}
